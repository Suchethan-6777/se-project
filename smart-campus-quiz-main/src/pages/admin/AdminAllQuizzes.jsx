...
const AdminAllQuizzes = () => {
  ...
  const handleDeleteQuiz = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/api/admin/quizzes/${selectedQuiz.id}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button size="lg" onClick={() => navigate('/admin/quiz/create')} className="gap-2">
      ...
      {quizzes.length === 0 ? (
        <Card className="shadow-card">
          <CardContent className="py-12 text-center">
            ...
            <h3 className="text-xl font-semibold mb-2">No quizzes in the system</h3>
            <p className="text-muted-foreground mb-6">
              Create the first quiz to get started
            </p>
            <Button onClick={() => navigate('/admin/quiz/create')} className="gap-2">
              <Plus className="h-5 w-5" />
              Create Quiz
            </Button>
          </CardContent>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => navigate(`/admin/quiz/${quiz.id}/submissions`)}
                            className="gap-1"
                          >
